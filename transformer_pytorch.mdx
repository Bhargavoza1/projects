...
1. The dot products between the query vectors and key vectors are computed to get raw attention scores: 

scores = $$QK^T$$

2. The raw attention scores are scaled by the square root of the dimension of the key vectors $$d_k$$. This helps in stabilizing the gradients during training:
